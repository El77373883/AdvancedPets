...
public class ShopGUI implements Listener {
    ...
    static {
        // Comunes
        for (EntityType t : new EntityType[]{EntityType.WOLF, EntityType.CAT, EntityType.HORSE,
            EntityType.COW, EntityType.PIG, EntityType.SHEEP, EntityType.CHICKEN,
            EntityType.LLAMA, EntityType.RABBIT, EntityType.FOX, EntityType.TURTLE,
            EntityType.BEE, EntityType.PANDA, EntityType.PARROT, EntityType.COD,
            EntityType.SQUID, EntityType.DOLPHIN, EntityType.FROG, EntityType.CAMEL,
            EntityType.ARMADILLO}) MOB_RARITY.put(t, Pet.Rarity.COMMON);
        // Raras
        for (EntityType t : new EntityType[]{EntityType.IRON_GOLEM, EntityType.POLAR_BEAR,
            EntityType.ZOMBIE_VILLAGER, EntityType.SPIDER, EntityType.CAVE_SPIDER,
            EntityType.PHANTOM, EntityType.PIGLIN, EntityType.SNOW_GOLEM})
            MOB_RARITY.put(t, Pet.Rarity.RARE);
        // Épicas
        for (EntityType t : new EntityType[]{EntityType.SKELETON, EntityType.ZOMBIE,
            EntityType.CREEPER, EntityType.BLAZE, EntityType.ENDERMAN, EntityType.WITCH,
            EntityType.GUARDIAN, EntityType.PIGLIN_BRUTE, EntityType.HOGLIN,
            EntityType.RAVAGER, EntityType.VEX, EntityType.VINDICATOR, EntityType.SHULKER,
            EntityType.MAGMA_CUBE, EntityType.SLIME, EntityType.ELDER_GUARDIAN})
            MOB_RARITY.put(t, Pet.Rarity.EPIC);
        // Legendarias
        for (EntityType t : new EntityType[]{EntityType.ENDER_DRAGON, EntityType.WITHER,
            EntityType.WARDEN}) MOB_RARITY.put(t, Pet.Rarity.LEGENDARY);
    }
    ...
    public void open() {
        ...
        for (int i = 0; i < Math.min(mobs.size(), slots.length); i++) {
            ...
            double price = plugin.getEconomyManager().getPetPrice(type, currentTab);
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (!event.getView().getTitle().equals(color("§6§l✦ §e§lTienda de Mascotas §6§l✦"))) return;
        ...
        for (EntityType type : getMobsByRarity(currentTab)) {
    ...
    private Material getMobIcon(EntityType type) {
        switch (type) {
            ...
            case LLAMA: return Material.CARPET;
